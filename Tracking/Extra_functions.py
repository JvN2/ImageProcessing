...
def plot_link_traces(image, pp_df, name):
    try:
        directory = fr"links"
        os.mkdir(directory)
    ...
    tio.format_plot(r'x (pix)', r'y (pix)', title='Filtered image with traces', aspect=1.0, scale_page=0.5,
                    save=fr'filtered image with traces.png')
    plt.cla()
    return
