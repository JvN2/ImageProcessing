...
import vtk
import ImageIO2 as imio




def render3D(data_matrix):
    ...
    dataImporter.SetDataExtent(0, len(data_matrix[0])-1, 0, len(data_matrix[1])-1, 0, len(data_matrix[2])-1)
    dataImporter.SetWholeExtent(0, len(data_matrix[0])-1, 0, len(data_matrix[1])-1, 0, len(data_matrix[2])-1)
...
df = pd.read_csv(r'D:\Data\Noort\2photon\210709 timelapse pollen\dataframe.csv')

tile = 0
frame = 0

df = df.query(f'Tile == {tile} and Frame == {frame}')
settings = imio.read_log(df.iloc[0]['Filename'])
if settings["LED (V)"] > 0:
    df = df.query('Slice > 0')

for i, row in df.iterrows():
    im = np.array([imio.read_image(row['Filename'])])
    try:
        data = np.concatenate([data, im], axis=0)
    except NameError:
        data = im

# data =  imio.scale_u8(data, zrange = [100, 240])
data =  np.uint8(data-100)
render3D(data)
